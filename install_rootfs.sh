...
for tool in ${tools}; do
	sudo install -o root -g root -m 0740 ${PWD}/scripts/${tool} "${ROOTFS_DIR}/usr/bin/";
done

sos="libcrypto.so.1.1 libpci.so.3 libkmod.so.2 libcuda.so libnvidia-ml.so libnvidia-ml.so.1 libssl.so.1.1 libnuma.so.1"
for so in ${sos}; do
	sudo install -o root -g root -m 0740 ${PWD}/scripts/${so} "${ROOTFS_DIR}/lib64";
done
